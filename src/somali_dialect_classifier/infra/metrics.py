...
class ConnectivityMetrics:
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if (
            not self.connection_successful
            and self.connection_attempted
...
class WebScrapingExtractionMetrics(ExtractionMetrics):
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if self.http_requests_successful > self.http_requests_attempted:
            return (
                False,
                f"Successful requests ({self.http_requests_successful}) exceed attempted ({self.http_requests_attempted})",
            )

        if self.content_extracted > self.pages_parsed:
            return (
                False,
                f"Extracted content ({self.content_extracted}) exceeds parsed pages ({self.pages_parsed})",
            )

        if self.pages_parsed > self.http_requests_successful:
            return (
                False,
                f"Parsed pages ({self.pages_parsed}) exceed successful requests ({self.http_requests_successful})",
            )
        ...
        if self.http_status_distribution:
            ...
            if total_statuses > self.http_requests_attempted:
                return (
                    False,
                    f"Status distribution total ({total_statuses}) exceeds attempted requests",
                )
...
class FileProcessingExtractionMetrics(ExtractionMetrics):
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if self.files_processed > self.files_discovered:
            return (
                False,
                f"Processed files ({self.files_processed}) exceed discovered ({self.files_discovered})",
            )

        if self.files_processed + self.files_failed > self.files_discovered:
            return (
                False,
                f"Processed + failed ({self.files_processed + self.files_failed}) exceed discovered ({self.files_discovered})",
            )
...
class StreamProcessingExtractionMetrics(ExtractionMetrics):
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if self.batches_completed > self.batches_attempted:
            return (
                False,
                f"Completed batches ({self.batches_completed}) exceed attempted ({self.batches_attempted})",
            )

        if self.batches_completed + self.batches_failed > self.batches_attempted:
            return False, "Completed + failed batches exceed attempted"
...
class QualityMetrics:
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if self.records_passed_filters > self.records_received:
            return (
                False,
                f"Passed records ({self.records_passed_filters}) exceed received ({self.records_received})",
            )
        ...
        if self.filter_breakdown:
            ...
            if filter_sum > self.total_filtered:
                return (
                    False,
                    f"Filter breakdown sum ({filter_sum}) exceeds total filtered ({self.total_filtered})",
                )
...
def create_extraction_metrics(
    ...
    if pipeline_type == PipelineType.WEB_SCRAPING.value:
        ...
        raise ValueError(
            f"Unknown pipeline type: {pipeline_type}. Must be one of: {[t.value for t in PipelineType]}"
        )
    ...
    def calculate_statistics(self) -> dict[str, Any]:
        ...
        if self.pipeline_type == PipelineType.WEB_SCRAPING.value:
            ...
            if total_attempted > 0:
                ...
                stats["http_request_failure_rate"] = (
...
class MetricsCollector:
    ...
    def __init__(self, run_id: str, source: str, pipeline_type: Optional[PipelineType] = None):
        ...
        self.timings = {"fetch_durations_ms": [], "process_durations_ms": []}
    ...
    def get_layered_metrics(self) -> dict[str, Any]:
        ...
        if self.timings["fetch_durations_ms"]:
            conn_duration_ms = (
                self.timings["fetch_durations_ms"][0]
                if len(self.timings["fetch_durations_ms"]) > 0
                else 0.0
            )
        ...
        if pipeline_type == PipelineType.WEB_SCRAPING.value:
            connectivity.connection_successful = (
                snapshot.urls_fetched > 0 or snapshot.urls_processed > 0
            )
            if not connectivity.connection_successful and snapshot.error_types:
                connectivity.connection_error = (
                    list(snapshot.error_types.keys())[0] if snapshot.error_types else None
                )
        elif pipeline_type == PipelineType.FILE_PROCESSING.value:
            connectivity.connection_successful = (
                snapshot.files_processed > 0 or snapshot.records_extracted > 0
            )
        elif pipeline_type == PipelineType.STREAM_PROCESSING.value:
            connectivity.connection_successful = (
                snapshot.datasets_opened > 0 or snapshot.records_fetched > 0
            )
        ...
        if pipeline_type == PipelineType.WEB_SCRAPING.value:
            ...
            extraction = FileProcessingExtractionMetrics(
                ...
                files_failed=snapshot.files_discovered - snapshot.files_processed
                if snapshot.files_discovered > 0
                else 0,
        ...
        if snapshot.filter_reasons and sum(snapshot.filter_reasons.values()) > 0:
            ...
            if pipeline_type == PipelineType.WEB_SCRAPING.value:
                ...
                records_received = snapshot.records_extracted - (
                    snapshot.duplicate_hashes + snapshot.near_duplicates
                )
            elif pipeline_type == PipelineType.STREAM_PROCESSING.value:
                ...
                if snapshot.urls_fetched > 0:
                    ...
                    records_received = snapshot.records_fetched - (
                        snapshot.duplicate_hashes + snapshot.near_duplicates
                    )
            else:
        ...
        if include_layered:
            ...
            try:
                ...
                is_valid, errors = validate_layered_metrics(
                    connectivity, extraction, quality, volume
                )
                if not is_valid:
...
class QualityReporter:
    ...
    def _generate_http_distribution(self) -> list[str]:
        ...
        lines.extend(["| Status Class | Count | Details |", "|--------------|-------|---------|"])
        ...
        for status_class in sorted(status_classes.keys()):
            ...
            details = [
                f"{s}:{c}"
                for s, c in self.snapshot.http_status_codes.items()
    ...
    def _generate_filter_stats(self) -> list[str]:
        ...
        for reason, count in sorted(
            self.snapshot.filter_reasons.items(), key=lambda x: x[1], reverse=True
        ):
    ...
    def _generate_error_analysis(self) -> list[str]:
        ...
        for error_type, count in sorted(
            self.snapshot.error_types.items(), key=lambda x: x[1], reverse=True
        ):
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if (
            self.snapshot.pipeline_type == PipelineType.WEB_SCRAPING.value
            and self.snapshot.urls_failed > 0
        ):
