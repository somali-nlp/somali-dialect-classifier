/**
 * Standalone QA Test Suite for Quality Insights Tab Rebuild (commit d0c37d6)
 * Tests core analytics logic without browser dependencies
 */

// Inline the computeQualityAnalytics function for standalone testing
...
function normalizeSourceName(source) {
    ...
    return source
        .replace('Wikipedia-Somali', 'Wikipedia')
        .replace('BBC-Somali', 'BBC')
        .replace('HuggingFace-Somali-c4-so', 'HuggingFace')
        .replace('Sprakbanken-Somali', 'Språkbanken');
}

function computeQualityAnalytics(metrics = []) {
    ...
    metrics.forEach(metric => {
        ...
        totalCandidateRecords += records + rejected;
...
function testQualityHealthPulseKPIs() {
    ...
    const totalRecords = mockMetrics.reduce((sum, metric) => sum + (metric.records_written || 0), 0);
    const totalRejectedFromMock = mockMetrics.reduce((sum, metric) => {
        const rejected = Object.values(metric.filter_breakdown || {}).reduce((innerSum, value) => innerSum + (Number(value) || 0), 0);
        return sum + rejected;
    }, 0);
    const expectedAvgQuality = (totalRecords + totalRejectedFromMock) > 0
        ? totalRecords / (totalRecords + totalRejectedFromMock)
        : 0;
    ...
    logTest('KPI Cards', 'Total rejection count', actualRejected === totalRejectedFromMock,
        actualRejected === totalRejectedFromMock ? null : `Expected ${totalRejectedFromMock}, got ${actualRejected}`);
    ...
    logTest('KPI Cards', 'Quality threshold logic (holds amber when below 85%)', analytics.avgQualityRate < 0.85);
...
function testDynamicQualityBriefings() {
    ...
    if (bbcSource) {
        const bbcMetric = mockMetrics.find(metric => normalizeSourceName(metric.source || '').includes('BBC'));
        const expectedBBCQuality = bbcMetric
            ? bbcMetric.records_written / (bbcMetric.records_written + Object.values(bbcMetric.filter_breakdown || {}).reduce((sum, value) => sum + (Number(value) || 0), 0))
            : null;
        const bbcQualityMatch = expectedBBCQuality !== null && Math.abs(bbcSource.quality - expectedBBCQuality) < 0.001;
        logTest('Briefings', 'Per-source quality calculation', bbcQualityMatch,
            bbcQualityMatch ? null : `Expected ${(expectedBBCQuality || 0).toFixed(3)}, got ${bbcSource.quality.toFixed(3)}`);
...
function runAllTests() {
    ...
    const severity = testResults.failed === 0 ? 'APPROVED' :
                    passRate >= 95 ? 'APPROVED WITH MINOR ISSUES' :
                    passRate >= 85 ? 'NEEDS REVIEW' : 'REJECTED';
