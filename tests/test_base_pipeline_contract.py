...
class TestBasePipelineContract:
    ...
    def test_processor_implements_required_methods(self, processor_class):
        ...
        assert hasattr(processor, "_create_cleaner"), (
            f"{processor_class.__name__} missing _create_cleaner()"
        )
        assert hasattr(processor, "_extract_records"), (
            f"{processor_class.__name__} missing _extract_records()"
        )
        assert hasattr(processor, "_get_source_type"), (
            f"{processor_class.__name__} missing _get_source_type()"
        )
        assert hasattr(processor, "_get_license"), (
            f"{processor_class.__name__} missing _get_license()"
        )
        assert hasattr(processor, "_get_language"), (
            f"{processor_class.__name__} missing _get_language()"
        )
    ...
    def test_processor_has_required_attributes(self, processor_class):
        ...
        assert hasattr(processor, "date_accessed"), (
            f"{processor_class.__name__} missing date_accessed"
        )
        ...
        assert hasattr(processor, "processed_dir"), (
            f"{processor_class.__name__} missing processed_dir"
        )
        assert hasattr(processor, "text_cleaner"), (
            f"{processor_class.__name__} missing text_cleaner"
        )
        assert hasattr(processor, "silver_writer"), (
            f"{processor_class.__name__} missing silver_writer"
        )
        assert hasattr(processor, "logger"), f"{processor_class.__name__} missing logger"
        assert hasattr(processor, "log_frequency"), (
            f"{processor_class.__name__} missing log_frequency"
        )
    ...
    def test_source_naming_convention(self, processor_class):
        ...
        assert "-" in processor.source, (
            f"{processor_class.__name__} source doesn't follow 'Provider-Language' pattern"
        )
    ...
    def test_get_source_type_returns_string(self, processor_class):
        ...
        assert isinstance(source_type, str), (
            f"{processor_class.__name__}._get_source_type() must return string"
        )
    ...
    def test_get_license_returns_string(self, processor_class):
        ...
        assert isinstance(license_str, str), (
            f"{processor_class.__name__}._get_license() must return string"
        )
    ...
    def test_get_language_returns_iso_code(self, processor_class):
        ...
        assert isinstance(language, str), (
            f"{processor_class.__name__}._get_language() must return string"
        )
...
class TestRawRecord:
    ...
    def test_raw_record_metadata_defaults_to_empty_dict(self):
        ...
        record = RawRecord(title="Test", text="Text", url="https://example.com")
